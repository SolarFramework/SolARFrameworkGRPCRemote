...
class IMapsManager_grpcProxy:  public org::bcom::xpcf::ConfigurableBase, virtual public SolAR::api::service::IMapsManager {
  public:
    ...
    SolAR::FrameworkReturnCode backupMap(std::string const& mapUUID, std::vector<unsigned char>& map_information, std::vector<unsigned char>& cameraParameters, std::vector<unsigned char>& coordinate, std::vector<unsigned char>& covisibility_graph, std::vector<unsigned char>& identification, std::vector<unsigned char>& keyframes, std::vector<unsigned char>& pointcloud)     const     override;
    SolAR::FrameworkReturnCode restoreMap(std::string const& mapUUID, std::vector<unsigned char> const& map_information, std::vector<unsigned char> const& cameraParameters, std::vector<unsigned char> const& coordinate, std::vector<unsigned char> const& covisibility_graph, std::vector<unsigned char> const& identification, std::vector<unsigned char> const& keyframes, std::vector<unsigned char> const& pointcloud)     override;
