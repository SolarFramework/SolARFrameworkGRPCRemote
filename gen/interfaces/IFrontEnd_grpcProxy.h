...
class IFrontEnd_grpcProxy:  public org::bcom::xpcf::ConfigurableBase, virtual public SolAR::api::service::IFrontEnd {
  public:
    ...
    SolAR::FrameworkReturnCode backupMap(std::string const& accessToken, std::string const& mapUUID, std::vector<std::byte>& compressedZipData)     const     override;
    SolAR::FrameworkReturnCode restoreMap(std::string const& accessToken, std::string const& mapUUID, std::vector<std::byte> const& compressedZipData)     override;
