...
class IFrontEnd_grpcProxy:  public org::bcom::xpcf::ConfigurableBase, virtual public SolAR::api::service::IFrontEnd {
  public:
    ...
    SolAR::FrameworkReturnCode backupMap(std::string const& accessToken, std::string const& mapUUID, std::vector<unsigned char>& compressed_zip_data)     const     override;
    SolAR::FrameworkReturnCode restoreMap(std::string const& accessToken, std::string const& mapUUID, std::vector<unsigned char> const& compressed_zip_data)     override;
