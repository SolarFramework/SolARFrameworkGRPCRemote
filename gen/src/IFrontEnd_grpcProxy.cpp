...
SolAR::FrameworkReturnCode  IFrontEnd_grpcProxy::getStatusForMapProcessing(std::string const& keycloakToken, std::string const& mapUUID, SolAR::api::service::MapProcessingStatus& status, float& progress, std::string& resultingMapUUID) const
{
  ::grpc::ClientContext context;
  ::grpcIFrontEnd::getStatusForMapProcessingRequest reqIn;
  ::grpcIFrontEnd::getStatusForMapProcessingResponse respOut;
  #ifndef DISABLE_GRPC_COMPRESSION
  ...
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime start = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IFrontEnd_grpcProxy::getStatusForMapProcessing request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->getStatusForMapProcessing(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IFrontEnd_grpcProxy::getStatusForMapProcessing response received at " << to_simple_string(end) << std::endl;
  std::cout << "   => elapsed time = " << ((end - start).total_microseconds() / 1000.00) << " ms" << std::endl;
  #endif
  if (!grpcRemoteStatus.ok())  {
    std::cout << "getStatusForMapProcessing rpc failed." << std::endl;
    throw xpcf::RemotingException("grpcIFrontEndService","getStatusForMapProcessing",static_cast<uint32_t>(grpcRemoteStatus.error_code()));
  }
...
SolAR::FrameworkReturnCode  IFrontEnd_grpcProxy::getDataForMapProcessing(std::string const& keycloakToken, std::string const& mapUUID, std::vector<SRef<SolAR::datastructure::CloudPoint>>& pointCloud, std::vector<SolAR::datastructure::Transform3Df>& keyframePoses) const
{
  ::grpc::ClientContext context;
  ::grpcIFrontEnd::getDataForMapProcessingRequest reqIn;
  ::grpcIFrontEnd::getDataForMapProcessingResponse respOut;
  #ifndef DISABLE_GRPC_COMPRESSION
  ...
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime start = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IFrontEnd_grpcProxy::getDataForMapProcessing request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->getDataForMapProcessing(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IFrontEnd_grpcProxy::getDataForMapProcessing response received at " << to_simple_string(end) << std::endl;
  std::cout << "   => elapsed time = " << ((end - start).total_microseconds() / 1000.00) << " ms" << std::endl;
  #endif
  if (!grpcRemoteStatus.ok())  {
    std::cout << "getDataForMapProcessing rpc failed." << std::endl;
    throw xpcf::RemotingException("grpcIFrontEndService","getDataForMapProcessing",static_cast<uint32_t>(grpcRemoteStatus.error_code()));
  }
