...
SolAR::FrameworkReturnCode  ITrackableLoader_grpcProxy::loadTrackable(SRef<SolAR::datastructure::Trackable>& trackable)
{
  ...
  xpcf::grpcCompressionInfos proxyCompressionInfo = xpcf::deduceClientCompressionInfo(m_serviceCompressionInfos, "loadTrackable", m_methodCompressionInfosMap);
  ...
  std::cout << "====> ITrackableLoader_grpcProxy::loadTrackable request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->loadTrackable(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> ITrackableLoader_grpcProxy::loadTrackable response received at " << to_simple_string(end) << std::endl;
