...
SolAR::FrameworkReturnCode  IWorldGraphLoader_grpcProxy::load(std::vector<SRef<SolAR::datastructure::Trackable>>& trackables)
{
  ...
  std::cout << "====> IWorldGraphLoader_grpcProxy::load request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->load(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IWorldGraphLoader_grpcProxy::load response received at " << to_simple_string(end) << std::endl;
