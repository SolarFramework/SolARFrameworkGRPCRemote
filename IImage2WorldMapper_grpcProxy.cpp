...
SolAR::FrameworkReturnCode  IImage2WorldMapper_grpcProxy::map(std::vector<SolAR::datastructure::Point2Df> const& digitalPoints, std::vector<SolAR::datastructure::Point3Df>& worldPoints)
{
  ...
  xpcf::grpcCompressionInfos proxyCompressionInfo = xpcf::deduceClientCompressionInfo(m_serviceCompressionInfos, "map", m_methodCompressionInfosMap);
  ...
  std::cout << "====> IImage2WorldMapper_grpcProxy::map request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->map(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IImage2WorldMapper_grpcProxy::map response received at " << to_simple_string(end) << std::endl;
