...
SolAR::FrameworkReturnCode  IUndistortPoints_grpcProxy::undistort(std::vector<SolAR::datastructure::Point2Df> const& inputPoints, std::vector<SolAR::datastructure::Point2Df>& outputPoints)
{
  ...
  xpcf::grpcCompressionInfos proxyCompressionInfo = xpcf::deduceClientCompressionInfo(m_serviceCompressionInfos, "undistort", m_methodCompressionInfosMap);
  ...
  std::cout << "====> IUndistortPoints_grpcProxy::undistort request sent at " << to_simple_string(start) << std::endl;
  #endif
  ::grpc::Status grpcRemoteStatus = m_grpcStub->undistort_grpc0(&context, reqIn, &respOut);
  #ifdef ENABLE_PROXY_TIMERS
  boost::posix_time::ptime end = boost::posix_time::microsec_clock::universal_time();
  std::cout << "====> IUndistortPoints_grpcProxy::undistort response received at " << to_simple_string(end) << std::endl;
